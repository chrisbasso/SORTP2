...
imprimir_menu () {
    ...
    echo -e "\t\t Opciones:";
    echo "";
    echo -e "\t\t\t a.  Dirección IP y Red";
    echo -e "\t\t\t b.  Dirección Router";
    echo -e "\t\t\t c.  Ver dispositivos disponibles en la red y comprobar conectividad";   
    echo -e "\t\t\t q.  Salir";
    echo "";
...
a_funcion () {
        ...
        echo -e "Red:"
        ip route | awk '{print $1}' | sed -n 3p
}

b_funcion () {
           imprimir_encabezado "\tOpción b";
           echo -e "Router"
           ip route | grep "via" | awk '{print $3}'
}

c_funcion () {
          imprimir_encabezado "\tOpción c";
          echo "Dispositivos disponibles en la red:"
          router=$(ip route | grep "via" | awk '{print $3}')
          sudo nmap -sP $router/24 | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}'
          echo "Comprobando conectividad:"
          for ip in $(sudo nmap -sP $router/24 | grep -oE '([0-9]{1,3}\.){3}[0-9]{1,3}')
          do
                echo "---------------------------------------------------"
                ping -c 4 $ip;
          done
